...
import gdown
import os
from st_audiorec import st_audiorec  # Import st_audiorec for recording
...
def load_model_from_zip(zip_filename, model_filename):
    ...
    return load_model_from_file(model_filename)
...
if audio_bytes is not None:
    temp_audio_recorded_path = "./temp_recorded_audio.wav"
    with open(temp_audio_recorded_path, "wb") as f:
        f.write(audio_bytes)

    audio_data, sr = librosa.load(temp_audio_recorded_path, sr=None)
    predicted_class_name, confidence = process_and_predict(audio_data)
